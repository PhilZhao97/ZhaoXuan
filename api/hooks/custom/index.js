        ...
        '/*': {
          ...
          fn: async function(req, res, next){
            ...
            if (req.method === 'GET') {
              ...
              // The `user` local is set explicitly to `undefined` here just to avoid having to
              // do `typeof user !== 'undefined'` checks in our views/layouts/partials.
              // > Note that, depending on the request, this may or may not be set to the
              // > logged-in user record further below.
              if (res.locals.user !== undefined) {
                throw new Error('Cannot attach view local `user`, because this view local already exists!  (Is it being attached somewhere else?)');
              }
            ...
            // Expose the user record as an extra property on the request object (`req.user`).
            // > Note that we make sure `req.user` doesn't already exist first.
            if (req.user !== undefined) {
              throw new Error('Cannot attach logged-in user as `req.user` because this property already exists!  (Is it being attached somewhere else?)');
            }
            ...
            // If this is a GET request, then also expose an extra view local (`<%= user %>`).
            // > Note that we make sure a local named `user` doesn't already exist first.
            // > Also note that we strip off any properties that correspond with protected attributes.
            if (req.method === 'GET') {
              if (res.locals.user !== undefined) {
                throw new Error('Cannot attach logged-in user as the view local `user`, because this view local already exists!  (Is it being attached somewhere else?)');
              }
